...
- `Kairos Stake Guard` : moteur de mise prudente.
...
6. `Bet Center` : décision `ADVICE`, `WATCH`, `NO_BET` ou `INSUFFICIENT_DATA`.
7. Journal immuable.
...
Heure, mode, fraîcheur, probabilités complètes, incertitude, facteurs, sources, version du modèle, version de `Kairos`, décision de risque et avertissement responsable.

## Hors portée MVP
Exécution de mise, martingale, bankroll réelle, tickets automatiques, promesse de gains et score exact comme produit principal.
